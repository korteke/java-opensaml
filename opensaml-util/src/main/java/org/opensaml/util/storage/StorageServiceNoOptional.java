 ...
 */
@ThreadSafe
public interface StorageServiceNoOptional extends InitializableComponent, DestructableComponent, IdentifiableComponent,
     ...
     * @param expiration    expiration for record or null for none
     * 
     ...
     * @param context       a storage context label
     * @param key           a key unique to context
     * @param version       if non-null, only return record if newer than supplied version
     * 
     ...
     * @param context       a storage context label
     * @param key           a key unique to context
     * @param value         updated value (may be null, indicating only the expiration should be updated)
     * @param expiration    updated expiration, null indicating no expiration
     * @param version       if non-null, only update if the current version matches this value
     * 
     * @return the version of the record after update, null if no record exists, or -1 if the version
    ...
    @Nullable public Integer updateString(@Nonnull @NotEmpty final String context,
            ...
            @Nullable final Long expiration, @Nullable final Integer version) throws IOException;
     ...
     * 
     * @param context       a storage context label
     * @param expiration    a new expiration timestamp, null indicating no expiration
     * 
     * @throws IOException  if errors occur in the cleanup process
     */
