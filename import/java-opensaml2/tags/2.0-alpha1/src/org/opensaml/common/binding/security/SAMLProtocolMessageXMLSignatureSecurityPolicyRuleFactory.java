...
/**
 * Factory for producing SAML security policy rules which validate the signature (if present) on SAML 1
 * and SAML 2 protocol messages.
 */
public class SAMLProtocolMessageXMLSignatureSecurityPolicyRuleFactory 
    ...
    /**
     *
     * SAML security policy rule which validates the signature (if present) on the {@link SAMLObject} which 
     * represents the SAML protocol message being processed.  If the message is not an instance of
     * {@link SignableSAMLObject}, then no processing is performed.  If signature validation is successful,
     * and the context issuer was not previously authenticated, then the context's issuer authentication
     * state will be set to <code>true</code>.
     */
    protected class SAMLProtocolMessageXMLSignatureSecurityPolicyRule extends BaseSAMLSignatureSecurityPolicyRule {
