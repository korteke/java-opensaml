 ...
 * 
 * The whole is constructed carefully to be multithreaded and threadsafe. It uses {@link java.lang.ref.SoftReference} to
 * allow caching with an LRU.
 * 
 */
public class MetadataCacheImpl implements MetadataCache, Runnable {
    ...
    private final Map<String, LookupEntry> uriEntryMap;

    /**
     * The unmarshaller factory gives us there wherewithall to get from Document to XMLObject
     */
    ...
    public MetadataCacheImpl(UnmarshallerFactory unmarshallerFactory, int lruSize, long defaultWaitSecs) {
        uriEntryMap = Collections.synchronizedMap(new HashMap<String, LookupEntry>());
    ...
    public void loadMetadata(String metadataURI, MetadataResolverFactory resolverFactory, MetadataFilter filter)
        ...
        uriEntryMap.put(metadataURI, entry);
    ...
    public EntitiesDescriptor retrieveEntities(String metadataURI) {
        XMLObject object = retrieveObject(metadataURI);

        if (object != null && object instanceof EntitiesDescriptor) {
    ...
    public EntityDescriptor retrieveEntity(String metadataURI) {
        ...
        if (object != null && object instanceof EntityDescriptor) {
    ...
    /**
     * Retrieves XMLObject from cache
     * 
     * @param metadataURI the URI of the metadata
     * 
     * @return the XMLObject retrieved from unmarshalling the resolved URI
     */
    private XMLObject retrieveObject(String metadataURI){
        XMLObject object = null;
        LookupEntry entry = uriEntryMap.get(metadataURI);
    ...
    public void removeMetadata(String metadataURI) {
        LookupEntry entry;
        entry = uriEntryMap.remove(metadataURI);
    ...
    /**
     * This is the reference which goes into the map. It contains a weak reference to the actual XML reference (so it
     * might be null).
     */
    private class LookupEntry {
        /*
         * Most of the caching smarts are contained within this reference. All of the methods can be called in multiple
         * threads and so we need to be careful about synchronization and even more about the java memory model which
         * does not guarantee write ordering when seen from mutiple threads.
         * 
         * Thus a frequent paradigm is to copy a private field into a local variable (under a mutex because of the
         * re-ordering constraints) and then do something with it. Others may be clearing the field...
         */
        ...
        /**
         * Generation count, to stop the clear thread from doing the wrong thing - it is incremented every time
        ...
        private XMLObject get() throws ResolutionException, UnmarshallingException, FilterException {
            ...
            /*
             * Java has a non intutitive memory model - loads can happen out of order. Hence it is possible to collect
             * the expirely date after we have collected the XMLObject. This is bad because the expirey date may be for
             * a more recent version of the object and we will return bad data. Hence we have to synchronize and (to
             * stop deadlock) we do so on an internal object
             */
            synchronized (mutex) {
                exp = expires;
                ref = reference;
            ...
            if (retVal == null) {
                /*
                 * Something has made the entry invalid - either a clear or a GC, or the expirey date. We will do an
                 * inline load and return the value immediately.
                 */
                retVal = load();
        ...
        /**
         * The element is about to be destroyed so lets clear up all our dangluing issues However we cannot just call
         * clear because that might trigger a reload...
         */
        private void delete() {
            //
        ...
        private XMLObject load() throws ResolutionException, UnmarshallingException, FilterException {
            /*
             * Load is either called because we are not lazy loading and the data has gone invalid, or because the user
             * needs the data and it is invalid. It is therefore serialized and we take a wee look at the data every
             * time we get the lock (in case wehave just got through loading it.
             * 
             * It is not sufficient for load to just set up the soft reference - it might have been garbage collected by
             * the time the call returns. Hence load sets up the soft reference *and* returns the value.
             * 
    ...
    /**
     * Helper fuction for loading. According to the spec cache data must be dropped if either cacheDuration or
     * validUntil. This iterates down the objects finding the earliest time when the parent should be made invalid.
     * 
    ...
    /**
     * Helper function for expires. Returns the earlier of two dates, with null being 'high;
     * 
     * @param first
     * @param second
    ...
    private void lruRemove(XMLObject object) {
        ...
        // No op for empty LRU list
        //
        if (object == null || lruList.length == 0)
            return;
    ...
    public void run() {
        ...
        while (keepLooping) {
            ...
            synchronized (uriEntryMap) {
                for (Map.Entry<String, LookupEntry> mapEntry : uriEntryMap.entrySet()) {
                    LookupEntry cacheEntry = mapEntry.getValue();
                    if (cacheEntry.getExpires() == null) {
                        entry = cacheEntry;
                        time = null;
                        break;
                    }
                    if (time == null || time.isAfter(cacheEntry.getExpires())) {
                        entry = cacheEntry;
                        time = cacheEntry.getExpires();
                    }
                }
            } // syncrhonized block
