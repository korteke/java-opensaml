...
public class HTTPPostSimpleSignEncoder extends HTTPPostEncoder {
    ...
    /** Class logger. */
    private final Logger log = Logger.getLogger(HTTPPostSimpleSignEncoder.class);
    
    /** Flag to indicate whether the SAML 2 protocol message should additionally be signed
     * using the XML Signature, in addition to SimpleSign. */
    ...
    /**
     * Generates the signature over the string of concatenated form control data as indicated by
     * the SimpleSign spec.
     * 
    ...
    protected String generateSignature(Credential signingCredential, String algorithmURI, String formData)
        ...
        if (log.isDebugEnabled()) {
            log.debug(String.format("Generating signature with key type '%s', algorithm URI '%s' over form control string '%s'",
