...
/**
 * A validator for instances of {@link Signature}, which validates that the signature meets
 * security-related requirements indicated by the SAML profile of XML Signature.
 */
public class SAMLSignatureProfileValidator implements Validator<Signature> {
    
    /** Class logger. */
    private static Logger log = Logger.getLogger(SAMLSignatureProfileValidator.class);
    ...
    public void validate(Signature signature) throws ValidationException {
        ...
        if (!(signature instanceof SignatureImpl)) {
            if (log.isInfoEnabled()) {
                log.info(String.format("Signature was not an instance of SignatureImpl, was '%s', " +
                        "validation not supported",  signature.getClass().getName()));
            }
            return;
        }
    ...
    /**
     * Validate an instance of {@link SignatureImpl}, which is in turn based on 
     * underlying Apache XML Security <code>XMLSignature</code> instance.
     * 
     * @param sigImpl the signature implementation object to validate
     * @throws ValidationException  thrown if the signature is not valid with respect
     *          to the profile
     */
     ...
     * @throws ValidationException thrown if the Signature does not contain exactly 1 Reference,
     *          or if there is an error obtaining the Reference instance
     */
     ...
     * 
     * The URI must either be null or empty (indicating that the entire enclosing document
     * was signed), or else it must be a local document fragment reference and point
     * to the SAMLObject parent via the latter's ID attribute value.
     * 
    ...
    protected void validateReferenceURI(String uri, String id) throws ValidationException {
        if (!DatatypeHelper.isEmpty(uri)) {
            if (!uri.startsWith("#")) {
                ...
                log.error(
                        String.format("Reference URI '%s' did not point to SignableSAMLObject with ID '%s'",
                                uri, id));
     ...
     * 
     * The Reference may contain at most 2 transforms.  One of them must
     * be the Enveloped signature transform.  An Exclusive Canonicalization
     * transform (with or without comments) may also be present. No other transforms
     * are allowed.
     * 
