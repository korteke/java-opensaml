 ...
 * 
 * <p>
 * If the issuer has been previously set in the security policy context by another rule, then that issuer is used to
 * build a criteria set via {@link #buildCriteriaSet(String, ServletRequest, XMLObject,  SecurityPolicyContext)}, and
 * then evaluated via {@link #evaluate(Object, CriteriaSet)}. If this trust evaluation is successful, the context
 * issuer authentication state will be set to <code>true</code>, otherwise it will be set to <code>false</code>.
 * In either case, rule processing is then terminated.
 * </p>
 * 
 * <p>
 * If no context issuer was previously set, then rule evaluation will proceed as described in
 * {@link #evaluateCertificateNameDerivedIssuers(X509Credential, ServletRequest, XMLObject,  SecurityPolicyContext)},
 * based on the currently configured certificate name evaluation options. If this method returns a non-null issuer
 * entity ID, it will be set as the issuer in the context, the context's issuer authentication state will be set to
 * <code>true</code> and rule processing is terminated. If the method returns null, the context issuer and issuer
 * authentication state will remain unmodified and rule processing continues.
 * </p>
 * 
 * <p>
 * Finally rule evaluation will proceed as described in
 * {@link #evaluateDerivedIssuers(X509Credential, ServletRequest, XMLObject, SecurityPolicyContext)}. This is primarily
 * an extension point by which subclasses may implement specific custom logic. If this method returns a non-null issuer
 * entity ID, it will be set as the issuer in the context, the context's issuer authentication state will be set to
 * <code>true</code> and rule processing is terminated. If the method returns null, the context issuer and issuer
 * authentication state will remain unmodified.
 * </p>
 */
...
//            } else {
//                log.info("Authentication via client certificate failed for context issuer entity ID '" + contextIssuer
//                        + "'");
...
//
//    /**
//     * Evaluate any candidate issuer entity ID's which may be derived from the credential or other request or message
//     * information.
//     * 
//     * <p>
...
//
//    /**
//     * Evaluate candidate issuer entity ID's which may be derived from the request credential's entity certificate and
...
//     * <ol>
//     * <li>The certificate subject DN string as serialized by the X500DNHandler configured via
//     * {@link ClientCertAuthRuleFactory#getX500DNHandler()} and using the output format indicated by
//     * {@link ClientCertAuthRuleFactory#getX500SubjectDNFormat()}.</li>
//     * <li>Subject alternative names of the types configured via {@link
//     * ClientCertAuthRuleFactory#getSubjectAltNames()}. Note that this is a LinkedHashSet, so the order of evaluation is
//     * the order or insertion.</li>
...
//     * {@link BaseTrustEngineRule#buildCriteriaSet(String, javax.servlet.ServletRequest, XMLObject,
//     * SecurityPolicyContext)} will be returned.
//     * </p>
//     * 
//     * @param requestCredential the X509Credential derived from the request
//     * @param request the protocol request
...
//     * {@link ClientCertAuthRuleFactory#getSubjectAltNames()}.
//     * 
//     * @param requestCredential the X509Credential derived from the request
//     * @param request the protocol request
//     * @param message the incoming message
