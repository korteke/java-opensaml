...
public class DecryptionSignedContentTest extends XMLObjectBaseTestCase {
    ...
    public void testDecryptAndVerifySignedElement() throws MarshallingException, ValidationException, 
            UnmarshallingException, EncryptionException, DecryptionException, 
        ...
        Element apacheResolvedElement = 
            IdResolver.getElementById(decryptedSignature.getDOM().getOwnerDocument(), idValue);
        assertNotNull("Apache ID resolver found no element", apacheResolvedElement);
        assertTrue("Apache ID resolver found different element",  
                decryptedSXO.getDOM().isSameNode(apacheResolvedElement));
