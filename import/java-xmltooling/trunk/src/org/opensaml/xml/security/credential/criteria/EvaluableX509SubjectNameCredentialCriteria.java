...
public class EvaluableX509SubjectNameCredentialCriteria implements EvaluableCredentialCriteria {
    ...
    /** Logger. */
    private static Logger log = Logger.getLogger(EvaluableX509SubjectNameCredentialCriteria.class);
    ...
    public Boolean evaluate(Credential target) {
        ...
        if (log.isDebugEnabled()) {
            log.debug(String.format("Evaluation of credential data '%s' against criteria data '%s' was: '%s'",
                    entityCert.getSubjectX500Principal().getName(X500Principal.RFC2253), 
                    subjectName.getName(X500Principal.RFC2253), result));
        }
        return result;
    }
