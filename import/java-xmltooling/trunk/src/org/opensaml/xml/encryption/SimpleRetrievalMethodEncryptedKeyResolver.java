 ...
 * by dereferencing {@link RetrievalMethod} children of the {@link KeyInfo} 
 * of the {@link EncryptedData} context.
 * 
 * The RetrievalMethod must have a <code>Type</code> attribute with the value of 
 * {@link EncryptionConstants#TYPE_ENCRYPTED_KEY}.  The <code>URI</code> attribute
 * value must be a same-document fragment identifier (via ID attribute).  Processing of transforms
 * children of RetrievalMethod is not supported by this implementation.
 */
public class SimpleRetrievalMethodEncryptedKeyResolver extends AbstractEncryptedKeyResolver {
    ...
    public Iterable<EncryptedKey> resolve(EncryptedData encryptedData) {
        ...
        for (RetrievalMethod rm : encryptedData.getKeyInfo().getRetrievalMethods()) { 
            ...
            if (rm.getTransforms() != null) {
                log.info("EncryptedKey RetrievalMethod has transforms, can not process");
                continue;
            }
    ...
    protected EncryptedKey dereferenceURI(RetrievalMethod rm) {
        ...
        if (DatatypeHelper.isEmpty(uri) || ! uri.startsWith("#") ) {
            log.info("EncryptedKey RetrievalMethod did not contain a same-document URI reference, can not process");
            return null;
        }
